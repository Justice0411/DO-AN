...
void ReadSensorsTask(void *pvParameters) {
    ...
    while(1) {
        ...
        sprintf(buffer, "%.1f;%.1f;%.1f;%.1f\n",
                ...
                data.light_values[2],
                data.light_values[3]);
        HAL_UART_Transmit(&huart2, (uint8_t*)"Sending data: ", 13, HAL_MAX_DELAY);
...
void Read_INA219(void *p) {
    ...
    while(1) {
        data.bus_voltage = INA219_ReadVoltage(&hi2c2);
        data.shunt_current = INA219_ReadCurrent(&hi2c2);
        data.power = data.bus_voltage * data.shunt_current;
...
void Process_INA219_Data(void *pvParameters) {
    SensorDatas receivedData;
    char buffer[200];
    
    while(1) {
        if(xQueueReceive(xSensorDataQueue, &receivedData, portMAX_DELAY) == pdPASS) {
					sprintf(buffer, "V: %.1f;I: %.1f;P: %.1f\n",
                    receivedData.bus_voltage,
                    receivedData.shunt_current,
                    receivedData.power);
